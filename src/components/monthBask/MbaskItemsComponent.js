...
class MbaskItemsComponent extends Component {
  constructor(props) {
    super(props);
    const { item } = props;
    this.state = {
  ...
  setData = async () => {
    ...
    const cartD = await localForage.getItem('bask');
    const cartData = cartD.value || [];
    const vocData = cartData.length > 0 ? cartData.find(item1 => item1._id == this.props.item._id) : {};
  ...
  unitsChange = async (e) => {
    ...
    const cartItemD = await localForage.getItem('baskItems');
  ...
  handleAddBasket = async () => {
    this.setState({ qty: 1 });
    const id = uuidv4()
    let itemData1 = this.state.item;
    itemData1['_id'] = id;
    itemData1['qty'] = 1;

    const baskItems = this.setAllData(itemData1);
    const baskData = await localForage.getItem('bask');

    const baskItem = await localForage.getItem('baskItems');
    let basketData = baskData.value || [];

    let baskItemD = baskItem.value || [];
    basketData.push(baskItems[0]);
    baskItemD.push(itemData1)
    this.props.handleAddBasketItem(this.state.item, basketData, baskItemD)
  }
        ...
        <div className="product-cart-wrap mb-30 wow animate__ animate__fadeIn animated" data-wow-delay=".1s">
          <div className="product-img-action-wrap">
            <div className="product-img product-img-zoom">
              ...
              <a className='me-2'><i className='fa fa-trash' onClick={() => this.handleBasketItemDelete(item)}> </i></a>
            </div>
            ...
            <div className="product-card-bottom">
              ...
              <div className="add-cart">
                {qty ?
                  ...
                  <button className='btn addcart-button text-white fw-bold' onClick={() => this.handleAddBasket()} ><i className='fa-solid fa-cart-shopping me-2'></i>Add Basket </button>
                }
              </div>
            </div>
          </div>
...
export default connect(mapStateToProps, mapDistachToProps)(MbaskItemsComponent);